...
document
  .getElementById('newServiceMobile')
  .addEventListener('click', modalNewRequest);
document
  .getElementById('newServiceDesktop')
  .addEventListener('click', modalNewRequest);
document
  .getElementById('listServiceMobile')
  .addEventListener('click', modalListRequest);
document
        ...
        <tr>
          ...
          <td>
            <div class="center">
              ...
              <i class="view-request-details-btn
...
document.addEventListener('click', async (event) => {
  if (event.target.classList.contains('view-request-details-btn')) {
...
export async function openRequestDetails(request) {
  ...
  const requestType =
    isMaintenance && request.maintenanceProblem > 0
      ? `
          <tr>
  ...
  if (request.requestStatus === 'Pendente') {
    ...
    if (budgetStatus === 'Pendente') {
      ...
      status = `<tr>
            <td colspan="2" style="text-align: center; background-color: var(--color-pending);">
              Orçamento aguardando Avaliação do Cliente
            </td>
          </tr>`;
      ...
      status = `<tr>
            <td colspan="2" style="text-align: center; background-color: var(--color-valid);">
              Orçamento aprovado. Marcaremos Data e Hora da execução da Ordem de Serviço.
            </td>
          </tr>`;
    } else {
      status = `<tr>
  ...
  document
    ...
    .addEventListener('click', async () => {
      ...
      if (budgetValue.value === 'Aprovado') {
        ...
        sendData.feedback = 'Orçamento Aprovado pelo Cliente';
      } else if (budgetValue.value === 'Reprovado') {
        sendData.budgetStatus = 'Reprovado';
        sendData.feedback = 'Orçamento Reprovado pelo Cliente';
      } else {
        showModalAlert(
          'Alert',
      ...
      <div class="form-content">
        ...
        <div class="form-center">
          <label class="label">Ambiente do Serviço:</label>
            ...
            <i class="bi bi-door-open toggle-icon-input"></i>
          </div>
        </div>
